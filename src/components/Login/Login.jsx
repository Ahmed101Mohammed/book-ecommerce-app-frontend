...
const Login = () =>
{
  ...
  const loginHandler = async(event) =>
  {
    ...
    {
      const responseOfUserData = await usersServices.getUserDataWithRefresh(response.accessToken)
      if(responseOfUserData.state)
      {
        dispatch(setNotification({state: true, message: 'Accessed successfuly'}))
        dispatch(setUser({
          accessToken: response.accessToken,
          userData: responseOfUserData.data
        }))
      ...
      {
        dispatch(setNotification({state: false, message: responseOfUserData.message}))
      }
    }
  ...
  return (
    <main className="flex w-full h-full">
      <div className="grid w-full xl:w-1/2 h-full">
        <form className="place-self-center w-3/4 h-auto shadow-lg p-5" onSubmit={loginHandler}>
          ...
          <button className="bg-teal-800 text-white	text-center font-bold capitalize w-full p-3 rounded">Login</button>
        </form>
      </div>
